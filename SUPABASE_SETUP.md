...
> **★2026-07-30 現在、この機能は保留にしている。** 無料枠の都合で、いったん
> 通信なしのPWAへ戻した（コミット参照）。アプリからログイン画面と接続コードは
> 外したが、**設計・SQL・Supabaseのプロジェクトはそのまま残してある**。
> 再開するときは、外したコミットを戻せば接続部分は復活する。
> 同期の下ごしらえ（1品1行の版管理）と書き出し・読み込みは、外さずに残している。
...
2. ~~**在庫を1品1行として扱えるようにする**~~（2026-07-27 実装済み）
   - 保存キー `fridge-leftovers-sync-v1` に `{ "item:eggs": { version, body, dirty } }`
     の形で持つ。**実体には項目を足していない**ので、在庫の判定・描画・書き出しの
     コードは一切触っていない
   - **印は変更のたびに付けず、保存のたびに前回の内容と比べて出す。** 在庫を消す
     場所がアプリ内に7箇所あり、全部へ細工を入れると足し忘れが必ず起きるため
   - 消えたものは墓石（`deletedAt`）にして相手へ伝える。ただし**サーバーが
     知らないまま消えたものは表から外すだけ**（足して消しただけで表が増え続けない）
